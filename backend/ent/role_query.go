...
func (_q *RoleQuery) QueryRolePerms() *RolePermQuery {
...
func (_q *RoleQuery) WithRolePerms(opts ...func(*RolePermQuery)) *RoleQuery {
	query := (&RolePermClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withRolePerms; query != nil {
		if err := _q.loadRolePerms(ctx, query, nodes,
			func(n *Role) { n.Edges.RolePerms = []*RolePerm{} },
			func(n *Role, e *RolePerm) { n.Edges.RolePerms = append(n.Edges.RolePerms, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadRolePerms(ctx context.Context, query *RolePermQuery, nodes []*Role, init func(*Role), assign func(*Role, *RolePerm)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roleperm.FieldRoleID)
	}
	query.Where(predicate.RolePerm(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.RolePermsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
