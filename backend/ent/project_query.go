...
func (_q *ProjectQuery) QueryOperationLogs() *OperationLogQuery {
...
func (_q *ProjectQuery) WithOperationLogs(opts ...func(*OperationLogQuery)) *ProjectQuery {
	query := (&OperationLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withOperationLogs; query != nil {
		if err := _q.loadOperationLogs(ctx, query, nodes,
			func(n *Project) { n.Edges.OperationLogs = []*OperationLog{} },
			func(n *Project, e *OperationLog) { n.Edges.OperationLogs = append(n.Edges.OperationLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProjectQuery) loadOperationLogs(ctx context.Context, query *OperationLogQuery, nodes []*Project, init func(*Project), assign func(*Project, *OperationLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(operationlog.FieldProjectID)
	}
	query.Where(predicate.OperationLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.OperationLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
