...
// QueryRolePerms chains the current query on the "role_perms" edge.
func (_q *PermissionQuery) QueryRolePerms() *RolePermQuery {
	query := (&RolePermClient{config: _q.config}).Query()
...
func (_q *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withRolePerms: _q.withRolePerms.Clone(),
...
// WithRolePerms tells the query-builder to eager-load the nodes that are connected to
// the "role_perms" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PermissionQuery) WithRolePerms(opts ...func(*RolePermQuery)) *PermissionQuery {
	query := (&RolePermClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRolePerms = query
	return _q
}
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := _q.withRolePerms; query != nil {
		if err := _q.loadRolePerms(ctx, query, nodes,
			func(n *Permission) { n.Edges.RolePerms = []*RolePerm{} },
			func(n *Permission, e *RolePerm) { n.Edges.RolePerms = append(n.Edges.RolePerms, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PermissionQuery) loadRolePerms(ctx context.Context, query *RolePermQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *RolePerm)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roleperm.FieldPermissionID)
	}
	query.Where(predicate.RolePerm(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(permission.RolePermsColumn), fks...))
	}))
